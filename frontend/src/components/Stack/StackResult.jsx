...
const StackResultsPage = () => {
    ...
    const totalQuestions = stackQuestions.length;
    const percentage = (score / totalQuestions) * 100;

    return (
        <div className="flex flex-col min-h-screen">
            <Header text="Stack Quiz Results" />
            ...
            <div className="flex flex-col items-center justify-center flex-grow px-4 py-6">
                ...
                <div className="w-full max-w-3xl bg-white text-black p-6 md:p-8 rounded-lg shadow-lg min-h-[500px]">
                    ...
                    <h2 className="text-xl md:text-2xl font-bold text-center text-gray-800 mb-3 flex items-center justify-center gap-2">
                        <BadgeCheck className="w-7 h-7 md:w-8 md:h-8 text-green-400" />
                        Stack Quiz Results
                    </h2>
                    ...
                    <div className="mt-5 space-y-4 max-h-72 md:max-h-96 overflow-y-auto px-2">
                        {stackQuestions.map((question, index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className="p-3 border rounded-lg shadow-md bg-gray-100"
                                >
                                    {/* Question */}
                                    ...
                                    {/* User Answer */}
                                    <p
                                        className={`mt-2 text-sm font-medium flex items-center gap-2 ${isCorrect ? "text-green-500" : "text-red-500"
                                            }`}
                                    >
                                        {isCorrect ? (
                                    ...
                                    {/* Explanation */}
                                    <div className="mt-2">
                                        <p className="text-gray-600 text-xs md:text-sm italic flex items-center gap-2">
                                            <Info className="w-4 h-4 text-gray-500" /> Explanation:
                                        </p>
                                        <p className="text-gray-700 text-xs md:text-sm italic mt-1">
...
export default StackResultsPage;
