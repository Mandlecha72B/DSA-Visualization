...
import MergeSortVisualizer from "./MergeSortVisualiser";
...
const SortingVisualizer = () => {
    ...
    const [quickSteps1, setQuickSteps1] = useState(null);
    const [quickSteps2, setQuickSteps2] = useState(null);
    const [mergeSteps1, setMergeSteps1] = useState(null);
    const [mergeSteps2, setMergeSteps2] = useState(null);
    ...
    const startSorting = async () => {
        ...
        await Promise.all([
            algo1 !== "Quick Sort" && algo1 !== "Merge Sort"
                ? algorithms[algo1](array1, setArray1, setHighlight1, setSorted1, setSteps1, 800, setExecutionTime1)
                : algo1 === "Quick Sort"
                    ? (() => {
                        const result = getQuickSortRecursiveSteps(array1);
                        setExecutionTime1(result.executionTime);
                        setQuickSteps1(result.steps);
                        return Promise.resolve();
                    })()
                    : (() => {
                        const result = getMergeSortSteps(array1);
                        setExecutionTime1(result.executionTime);
                        setMergeSteps1(result.treeSteps); // Or whatever state you use for Merge Sort steps
                        setMaxDepth(result.maxDepth); // Set max depth for Merge Sort
                        return Promise.resolve();
                    })(),

            algo2 !== "Quick Sort" && algo2 !== "Merge Sort"
                ? algorithms[algo2](array2, setArray2, setHighlight2, setSorted2, setSteps2, 800, setExecutionTime2)
                : algo2 === "Quick Sort"
                    ? (() => {
                        const result = getQuickSortRecursiveSteps(array2);
                        setExecutionTime2(result.executionTime);
                        setQuickSteps2(result.steps);
                        return Promise.resolve();
                    })()
                    : (() => {
                        const result = getMergeSortSteps(array2);
                        setExecutionTime2(result.executionTime);
                        setMergeSteps2(result.treeSteps); // Or whatever state you use for Merge Sort steps
                        setMaxDepth(result.maxDepth); // Set max depth for Merge Sort
                        return Promise.resolve();
                    })(),
        ]);
        ...
        <div className="min-h-screen w-screen mx-auto flex flex-col items-center bg-base-200 p-3   ">
            ...
            <div className="flex flex-nowrap justify-center gap-10 mt-3 w-[90%] max-h-[300px] overflow-y-auto overflow-x-auto">
                ...
                <div className="max-w-[90vw] overflow-x-auto">
                    ...
                    {algo1 === "Merge Sort" && mergeSteps1 && (
                        <MergeSortVisualizer array={array} sorting={sorting} steps={mergeSteps1} maxDepth={maxDepth} />
                    )}
                        ...
                        steps1.map((step, stepIndex) => (
                            <div key={stepIndex} className="mb-3">
                                <h4 className="text-sm text-gray-500">
                                    Pass {stepIndex + 1} (Comparisons: {step.comparisons})
                                </h4>
                ...
                <div className="max-w-[90vw] overflow-x-auto">
                    ...
                    {algo2 === "Merge Sort" && mergeSteps2 && (
                        <MergeSortVisualizer array={array} sorting={sorting} steps={mergeSteps2} maxDepth={maxDepth} />
                    )}
...
//SORTING VISUALIZER ALONG WITH QUICK SORT
/*import React, { useState, useEffect } from "react";
import { ClipboardList } from "lucide-react"; // Import icon from Lucide React
import Tooltip from "../Tooltip";
import Modal from "react-modal";
...
//SORTING VISUALIZER WITHOUT QUICK SORT
/*import React, { useState, useEffect } from "react";
import { ClipboardList } from "lucide-react"; // Import icon from Lucide React
import Tooltip from "../Tooltip";
import Modal from "react-modal";
